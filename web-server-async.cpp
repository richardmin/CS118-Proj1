...
int main(int argc, char* argv[])
{
  ...
  if (listen(sockfd, 10) == -1) {
    ...
    while (1) {
        ...
        if(numReady == 0)
        {
        	//nothing read, can handle automated disconnects here
        }
        else
        {
	        for (s = 0; s <= maxSockfd; s++) 
	        { //http://www.martinbroadhurst.com/server-examples.html#select-server
	            if (FD_ISSET(s, &readFds)) 
	            {
	                if (s == sockfd) { //listen socket
	                    /* New connection */
	                    int newsock;
	                    ...
	                    if (newsock == -1) {
				            ...
				            // update maxSockfd
				            if (maxSockfd < newsock)
				              maxSockfd = newsock;

				            // add the socket into the socket set
				            FD_SET(newsock, &watchFds);
	                    }
	                }
	                else { //normal socket
